...
def getInstances():
    ...
    for region in regions:
        ...
        if len(serverList["Reservations"]) != 0:
                ...
                for server in instances["Instances"]:        
                    ...
                    if instnaceState['Name'] == "running":
                        if "Tags" in server.keys():
                            ...
                            logging.info(f'In {region} region, {server["InstanceId"]} has no tags.')        
                    else:
            ...
            serverCount += 1        
    if serverCount == 0:
        logging.info(f"No Servers in any region")        
